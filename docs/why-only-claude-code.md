...
Readers who came in from [Everything Claude Code](https://github.com/affaan-m/everything-claude-code) often arrive with a fair question. ECC supports six harnesses (Claude Code, Cursor, Codex, OpenCode, Gemini, Antigravity), so why does Solo Stack only ship `.claude/` configs? Why no `.cursor/`, no `.codex/`, no `.gemini/`?

The question is the natural next one after looking at the [comparison table](comparing-stacks.md) and noticing that the **Multi-harness** row is one of the dimensions where ECC has wider coverage than this repo.
...
Each additional harness multiplies the maintenance surface without proportionally multiplying the value, *for a solo operator's stack*. ECC absorbs the multi-harness cost across 170+ contributors who collectively maintain it. A single-author curated stack like Solo Stack cannot absorb that cost without diluting the curation that is the value.

ECC is the catalogue across harnesses; Solo Stack is the curation layer over Claude Code specifically. Multi-harness in the curation layer would dilute the curation.
...
**Drift between harnesses.** Cursor's rules format, Codex's rules format, OpenCode's, Gemini's, Antigravity's all evolve independently of Claude Code's. A snapshot of all six is a maintenance bet against five independent rate-of-change vectors. Solo Stack's value depends on its content matching how the operator works, and that match has to be re-verified every time any harness ships a config-format change. With one harness it's a tractable check; with six it becomes a quarterly chore that gets skipped, and skipped chores become drift.
...
The single-harness scope is a maintainer constraint, not a stance against other harnesses or their users.
...
The "When this might change" section below is real, the threshold is reachable, and if a contributor crosses it the repo grows. The intent here isn't to keep other harnesses out of the repo permanently, only to be honest about what one maintainer can keep current.

## What to do if you use a non-Claude-Code harness

A short triage list, since this question lands often.

If you want the cookbook recipes' patterns but on Cursor or Codex, the recipes themselves are mostly framework-agnostic. Stripe Connect, WhatsApp Cloud API, Cloudflare Tunnel, Sentry, and the rest don't care which harness you run. Read the recipe; the implementation is portable.
...
If you want one harness with everything (Solo Stack content adapted across Cursor, Codex, OpenCode, Gemini, Antigravity, all maintained), the pointer is back to ECC's catalogue, not back here.
...
No such commitment exists today. Until it does, the maintainer would prefer to refer multi-harness readers to ECC, which already covers six harnesses with the contributor base to maintain that coverage, rather than dilute Solo Stack by attempting it alone.
...
The curation is the value, and that's where the maintainer's time goes.
