...
| 3 | **graphify** ([stack/graphify.md](../stack/graphify.md)) | **Recommended** (operator-private until v1.1) | Run per client folder so a knowledge graph of Client A stays scoped to Client A. The cross-client query is opt-in, not default. Skip if you do not have access — the per-client Obsidian notes still cover scoped memory. |
