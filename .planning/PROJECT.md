...
A curated toolkit and operator playbook for solo founders running multiple AI products in parallel using Claude Code. Documents which stack to install (Everything Claude Code, Toprank, Frontend-Design, OMEGA Memory, Obsidian, curated MCP set) and how to compose those parts into real shipping workflows. It is not a fork, not a redistribution, not a runtime — it is the install path, the workflows, the case studies, and the credits chain that pulls one operator's output across seven products into a single replicable stack.
