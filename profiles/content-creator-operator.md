...
What you actually want: cadence without becoming a content company. The Tuesday morning shape: open the laptop, decide whether today is a content day or a product day. If content: open `~/Brain/Content/`, pick the topic from the backlog, draft the script, set up the camera. If product: open the project, ship a feature, write the build-log post that becomes Wednesday's short-form clip. The cost you keep paying is mode-switching — the cognitive jump from "directing a video" to "debugging a webhook" is what makes most creator-builders give up on one of the two engines within six months.
...
| 3 | **graphify** ([stack/graphify.md](../stack/graphify.md)) | **Recommended** (operator-private until v1.1) | Run across `~/Brain/Content/` and `~/Brain/Projects/` — the knowledge graph that surfaces overlap between content topics and product threads is the use case for this profile. Skip if you do not have access — Obsidian's Dataview queries cover most of the same ground until graphify ships publicly. |
